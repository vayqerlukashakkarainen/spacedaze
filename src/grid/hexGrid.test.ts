...
test("hexRange - radius 2", () => {
	...
	assert(
		range.length === 19,
		"Range of radius 2 should have 19 hexes (1+6+12)"
	);
});
...
test("HexGrid - correct number of cells", () => {
	...
	// For axial coordinates, we expect exactly width * height cells
	const cellCount = grid.getAllCells().length;
	assert(
		cellCount === width * height,
		`Should have exactly ${width * height} cells, got ${cellCount}`
	);
});
...
test("deserializeGrid - roundtrip", () => {
	...
	// Check config
	assert(restored.config.width === original.config.width, "Width should match");
	assert(
...
test("gridCollision - tracks current cell", () => {
	...
	const entity = k.add([k.pos(centerPos), gridCollision("track_test")]) as any;
...
test("gridCollision - getCellProperties", () => {
	...
	const centerPos = grid.hexToScreen(hexCoord(5, 5));
	const entity = k.add([k.pos(centerPos), gridCollision("props_test")]) as any;
