...
/**
 * Convert hex coord with layer to string key for maps/storage
 * Matches HexGrid's cell key format: "q,r_L{layer}"
 */
export function hexKey(hex: HexCoord, layer: number): string {
	return `${hex.q},${hex.r}_L${layer}`;
}
...
/**
 * Rotate hex coordinate around origin by 60° steps
 * @param coord - Coordinate to rotate
 * @param steps - Number of 60° clockwise rotations (0-5)
 */
export function rotateHexCoord(coord: HexCoord, steps: number): HexCoord {
	...
	// Each step is 60° clockwise rotation in cube coordinates
	for (let i = 0; i < steps; i++) {
		// Rotation matrix: (q,r,s) -> (-s,-q,-r)
		const newQ = -s;
		const newR = -q;
		s = -r;
		q = newQ;
