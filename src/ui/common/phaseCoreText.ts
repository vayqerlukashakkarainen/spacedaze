...
export interface UiPhaseCoreValue {
	phaseCores: number
}

export type UiPhaseCoreTextPart = string | UiPhaseCoreValue

export interface UiPhaseCoreTextProps {
	parts: readonly UiPhaseCoreTextPart[]
	pos?: Vec2
	size?: number
	color?: Color
	valueColor?: Color
...
export function createUiPhaseCoreText(
	parent: GameObj,
	props: UiPhaseCoreTextProps
) {
	const size = props.size ?? UI_FONT_SIZES.small
	const color = props.color ?? k.rgb(...UI_COLORS.text)
	const valueColor = props.valueColor ?? k.rgb(...UI_COLORS.phaseCore)
	...
	for (const part of props.parts) {
		...
		const valueText = `${part.phaseCores}`
		const measurement = measureText(valueText, size)
		measuredParts.push({
			phaseCores: part.phaseCores,
	...
	for (const part of measuredParts) {
		...
		root.add([
			k.sprite("phase_core", { width: iconSize, height: iconSize }),
