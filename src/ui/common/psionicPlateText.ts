...
export interface UiPsionicPlateValue {
	psionicPlates: number
}

export type UiPsionicPlateTextPart = string | UiPsionicPlateValue

export interface UiPsionicPlateTextProps {
	parts: readonly UiPsionicPlateTextPart[]
	pos?: Vec2
	size?: number
	color?: Color
	valueColor?: Color
...
export function createUiPsionicPlateText(
	parent: GameObj,
	props: UiPsionicPlateTextProps
) {
	const size = props.size ?? UI_FONT_SIZES.small
	const color = props.color ?? k.rgb(...UI_COLORS.text)
	const valueColor = props.valueColor ?? k.rgb(...UI_COLORS.psionicPlate)
	...
	for (const part of props.parts) {
		...
		const valueText = `${part.psionicPlates}`
		const measurement = measureText(valueText, size)
		measuredParts.push({
			psionicPlates: part.psionicPlates,
	...
	for (const part of measuredParts) {
		...
		root.add([
			k.sprite("psionic_plate", { width: iconSize, height: iconSize }),
