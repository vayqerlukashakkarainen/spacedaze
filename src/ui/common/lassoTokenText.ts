...
export interface UiLassoTokenValue {
	lassoTokens: number
}

export type UiLassoTokenTextPart = string | UiLassoTokenValue

export interface UiLassoTokenTextProps {
	parts: readonly UiLassoTokenTextPart[]
	pos?: Vec2
	size?: number
	color?: Color
	valueColor?: Color
...
export function createUiLassoTokenText(
	parent: GameObj,
	props: UiLassoTokenTextProps
) {
	const size = props.size ?? UI_FONT_SIZES.small
	const color = props.color ?? k.rgb(...UI_COLORS.text)
	const valueColor = props.valueColor ?? k.rgb(...UI_COLORS.lassoToken)
	...
	for (const part of props.parts) {
		...
		const valueText = `${part.lassoTokens}`
		const measurement = measureText(valueText, size)
		measuredParts.push({
			lassoTokens: part.lassoTokens,
	...
	for (const part of measuredParts) {
		...
		root.add([
			k.sprite("salvage_lasso", { width: iconSize, height: iconSize }),
